...
export const FlashcardsPage: React.FC = () => {
  ...
  const [sessionStats, setSessionStats] = useState({ total: 0, remembered: 0, forgotten: 0 });
  ...
  const dragOpacity = useTransform(dragX, [-200, -150, 0, 150, 200], [0.5, 0.8, 1, 0.8, 0.5]);
  ...
  const currentDeck = useMemo(() => {
    ...
    if (dueOnly) {
      ...
      filtered = filtered.filter(w => new Date(w.nextReviewDate).getTime() <= now);
    }
  ...
  const activeCard = currentDeck[currentIndex] || null;

  // Khởi tạo/Reset session khi chuyển chủ đề hoặc chuyển bộ lọc
  useEffect(() => {
    setSessionStats({
  ...
  const progressPct = currentDeck.length > 0 ? Math.round(((currentIndex) / currentDeck.length) * 100) : 0;
  ...
  }, [activeCard, isListening, voiceSupported, showSummary, currentDeck]);
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 w-full md:w-auto">
          ...
          {!isRestudyMode && (
            <div className="flex items-center gap-3 bg-slate-50/50 dark:bg-slate-900/30 px-4 py-2.5 rounded-2xl border border-slate-100/50 dark:border-slate-800/30">
              ...
              />
              <label htmlFor="dueOnlyToggle" className="text-xs font-bold text-slate-500 dark:text-slate-400 cursor-pointer select-none">
        ...
        {currentDeck.length > 0 && !showSummary && (
          <div className="text-left md:text-right flex flex-col items-start md:items-end bg-white/50 dark:bg-slate-900/50 px-5 py-3 rounded-2xl border border-slate-100 dark:border-slate-800/50">
            ...
            <p className="text-xl font-serif-title text-indigo-600 dark:text-indigo-400 mt-1">
              {currentIndex + 1} <span className="text-sm font-sans font-medium text-slate-400">/ {currentDeck.length}</span>
            </p>
          </div>
        )}
      </div>
      ...
      {showSummary ? (
        <div className="w-full max-w-xl mx-auto bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-[2.5rem] p-8 md:p-12 border border-white/60 dark:border-slate-800/80 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] text-center flex flex-col items-center gap-6">
          ...
          <div className="space-y-2">
            <h2 className="text-3xl font-serif-title text-slate-800 dark:text-slate-100">
              {isRestudyMode ? 'Hoàn thành ôn từ khó!' : 'Hoàn thành phiên học!'}
            </h2>
          ...
          <div className="grid grid-cols-3 gap-4 w-full bg-slate-50/50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800/50 p-6 rounded-2xl">
            <div className="flex flex-col items-center">
              <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Đã ôn</span>
              <span className="text-2xl font-bold text-slate-800 dark:text-slate-200 mt-1">{sessionStats.total}</span>
            </div>
            <div className="flex flex-col items-center border-x border-slate-200/50 dark:border-slate-800/50">
              <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Đã nhớ</span>
              <span className="text-2xl font-bold text-teal-600 dark:text-teal-400 mt-1">{sessionStats.remembered}</span>
            </div>
            <div className="flex flex-col items-center">
              <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Độ chính xác</span>
              <span className="text-2xl font-bold text-indigo-600 dark:text-indigo-400 mt-1">
                {sessionStats.total > 0 ? Math.round((sessionStats.remembered / sessionStats.total) * 100) : 0}%
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full mt-2">
            {sessionWrongIds.length > 0 && (
              <button
                onClick={() => {
                  const wrongWords = words.filter(w => sessionWrongIds.includes(w.id));
        ...
        <div className="w-full flex flex-col items-center gap-8">
          ...
          <div className="w-full flex items-center justify-center relative px-4">
            ...
            <motion.div
              ...
              style={{ x: dragX, rotate: dragRotate, opacity: dragOpacity, cursor: 'grab' }}
          ...
          <div className="flex gap-6 w-full max-w-md">
            ...
            >
              ...
              <span className="text-[11px] font-bold uppercase tracking-widest">Chưa nhớ (A)</span>
            </button>
            <button
            ...
            >
              ...
              <span className="text-[11px] font-bold uppercase tracking-widest">Đã nhớ (D)</span>
            </button>
          </div>
        ...
        <div className="bg-white dark:bg-slate-950 p-12 text-center rounded-[2.5rem] border border-slate-200/40 dark:border-slate-850 shadow-sm w-full max-w-md">
          ...
          <h3 className="text-xl font-bold text-slate-850 dark:text-slate-200 mt-4">
            {dueOnly ? 'Không có từ nào cần ôn tập!' : 'Không tìm thấy thẻ vựng!'}
          </h3>
      ...
      {totalWordsCount > 0 && (
        <div className="w-full bg-white/60 dark:bg-slate-950/60 backdrop-blur-xl p-6 rounded-[2rem] border border-white/60 dark:border-slate-800/60 shadow-sm flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Phân bổ hộp ghi nhớ (Leitner)</span>
            <span className="text-xs font-medium text-slate-450 dark:text-slate-500">Tổng cộng: {totalWordsCount} từ</span>
          </div>
            ...
            {boxDistribution.map((count, index) => {
              ...
              return (
                <div key={index} className="flex flex-col items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400">Hộp {index + 1}</span>
                  <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold">{count}</span>
                </div>
              );
            })}
          </div>
